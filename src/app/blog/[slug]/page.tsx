...
import { fetchBlogBySlug } from "@/app/data/page";  // Make sure this is typed correctly
...
interface BlogPageProps {
  params: { slug: string };  // `slug` is required and should be a string
}
...
const BlogPage = ({ params }: BlogPageProps) => {
  ...
  useEffect(() => {
    const fetchBlogData = async () => {
      try {
        const blogData = await fetchBlogBySlug(params.slug); // Fetch blog data by slug
        if (blogData) {
          setBlog(blogData);
        } else {
      ...
      <div className="container mx-auto py-12 px-6 sm:px-8 md:px-16 lg:px-40">
        ...
        <div className="mt-12">
          ...
          <form onSubmit={handleCommentSubmit} className="mt-4">
            ...
            <button
              type="submit"
              className="px-6 py-2 bg-[#48cfad] text-black text-xl sm:text-2xl rounded-[34px] hover:bg-green-300 font-bold"
            >
              Submit
            </button>
          </form>
